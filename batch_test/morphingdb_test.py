...
IMAGE_TABLE = 'image_table_10000'
IMAGE_VECTOR_TABLE = 'image_vector_table_10000'

IMAGE_TEST_FILE = 'result/morphingdb_batch_image_no_thread_{}_batch_test_modify.json'
IMAGE_VECTOR_TEST_FILE = 'result/morphingdb_batch_image_vector_no_thread_{}_batch_test_modify.json'
timing_data_image = []
timing_data_vector = []
...
def create_model():
    ...
    vec_model_path = os.path.join(pre_path, 'models/resnet18_vec.pt')

    cur.execute("select * from model_info where model_name = 'defect_vec';")
    res = cur.fetchall()
    if len(res) == 0:
        cur.execute("select create_model('defect_vec', '{}', '', '');".format(vec_model_path))
    conn.commit()
    conn.close()
...
def image_batch_test():
    for batch_size in BATCH_SIZE:
        ...
        #     sql = "select predict_batch_float8('defect', 'gpu', image_path) over (rows between current row and {} following) from image_table_10000 limit {};".format(batch_size-1,count)
        #     cur.execute(sql)
        #     end = time.time()
        ...
        for count in IMAGE_COUNT_LIST:
            ...
            cur.execute("select register_process();")
            start = time.time()
            sql = "select predict_batch_float8('defect_vec', 'gpu', image_vector) over (rows between current row and {} following) from image_vector_table_10000 limit {};".format(batch_size-1, count)
            cur.execute(sql)
            end = time.time()
            cur.execute("select print_cost();")
            res = parse_timing_info(cur.fetchall()[0][0])
            ...
            try:
                with open(IMAGE_VECTOR_TEST_FILE.format(batch_size), 'r') as f_vector:
                    # 尝试加载现有数据
            ...
            with open(IMAGE_VECTOR_TEST_FILE.format(batch_size), 'w') as f_vector:
