...
class IMDBTest(PytorchAbstractClassifierFunction):
    ...
    def setup(self):
        ...
        model_urls = {
            "sst2": "../../models/traced_albert_vec.pt"  # noqa
        }
        ...
        def sst2():
            model = torch.load(model_urls["sst2"], map_location="cpu")
            return model
        ...
        self.sentencepiece_model.Load("../../models/spiece.model.old")
    ...
    def forward(self, tensor) -> pd.DataFrame:
        ...
        token_ids = token_ids.to(torch.device('cpu')).unsqueeze(0)
        attention_mask = attention_mask.to(torch.device('cpu')).unsqueeze(0)
        token_type_ids = token_type_ids.to(torch.device('cpu')).unsqueeze(0)
        position_ids = position_ids.to(torch.device('cpu')).unsqueeze(0)
        end = time.time()
        self.pre_time += (end-start)
