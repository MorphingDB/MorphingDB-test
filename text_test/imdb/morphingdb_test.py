...
TEXT_VECTOR_TABLE = 'imdb_test_vec'

TEXT_TEST_FILE = 'result/morphingdb_test.json'
TEXT_VECTOR_TEST_FILE = 'result/morphingdb_vector_test.json'
timing_data_image = []
timing_data_vector = []
...
def create_model():
    ...
    model_path = os.path.join(pre_path, 'models/traced_albert_vec.pt')

    cur.execute("select * from model_info where model_name = 'sst2_vec';")
    res = cur.fetchall()
    if len(res) == 0:
        cur.execute("select create_model('sst2_vec', '{}', '', '');".format(model_path))
...
def imdb_vector_test():
    for count in TEXT_COUNT_LIST:
        ...
        start = time.time()
        sql = "select predict_batch_float8('sst2_vec', 'gpu', comment_vec) over (rows between current row and 31 following ) from imdb_vector_test limit {};".format(count)
        cur.execute(sql)
        end = time.time()
