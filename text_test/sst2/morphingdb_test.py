...
TEXT_TABLE = 'nlp_test'
TEXT_VECTOR_TABLE = 'nlp_vector_test'
...
def sst2_vec_test():
    for count in TEXT_COUNT_LIST:
        ...
        cur.execute("select register_process();")
        start = time.time()
        sql = "select predict_batch_float8('sst2_vec', 'cpu', comment_vec) over (rows between current row and 15 following ) from nlp_vector_test limit {};".format(count)
        cur.execute(sql)
        end = time.time()
        cur.execute("select print_cost();")
        res = parse_timing_info(cur.fetchall()[0][0])
