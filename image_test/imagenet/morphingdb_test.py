...
IMAGE_TABLE = 'imagenet_table_10000'
IMAGE_VECTOR_TABLE = 'imagenet_vector_table_10000'

IMAGE_TEST_FILE = 'result/morphingdb_imagenet_test.json'
IMAGE_VECTOR_TEST_FILE = 'result/morphingdb_imagenet_test.json'
timing_data_image = []
timing_data_vector = []
...
def create_model():
    ...
    model_path = os.path.join(pre_path, 'models/resnet18_imagenet.pt')

    cur.execute("select * from model_info where model_name = 'defect_vec';")
    res = cur.fetchall()
    if len(res) == 0:
        cur.execute("select create_model('defect_vec', '{}', '', '');".format(model_path))
...
def imagenet_test():
    for count in IMAGE_COUNT_LIST:
        ...
        start = time.time()
        sql = "select predict_batch_float8('defect_vec', 'gpu', image_vector) over (rows between current row and 31 following) from " + IMAGE_VECTOR_TABLE + " limit {};".format(count)
        cur.execute(sql)
        end = time.time()
        ...
        try:
            with open(IMAGE_VECTOR_TEST_FILE, 'r') as f_vector:
                # 尝试加载现有数据
        ...
        # 写回文件
        with open(IMAGE_VECTOR_TEST_FILE, 'w') as f_vector:
