...
IMAGE_TEST_FILE = 'result/morphingdb_cifar10_test.json'
IMAGE_VECTOR_TEST_FILE = 'result/morphingdb_cifar10_vector_test.json'
timing_data_image = []
timing_data_vector = []
...
def create_model():
    ...
    model_path = os.path.join(pre_path, 'models/googlenet_cifar10.pt')
    print(model_path)
    cur.execute("select * from model_info where model_name = 'googlenet_cifar10';")
    res = cur.fetchall()
    if len(res) == 0:
        cur.execute("select create_model('googlenet_cifar10', '{}', '', '');".format(model_path))
...
def cifar10_test():
    for count in IMAGE_COUNT_LIST:
        ...
        start = time.time()
        sql = "select predict_batch_float8('googlenet_cifar10', 'cpu', image_vector) over (rows between current row and 31 following) from " + IMAGE_VECTOR_TABLE + " limit {};".format(count)
        cur.execute(sql)
        end = time.time()
