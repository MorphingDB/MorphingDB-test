...
IMAGE_VECTOR_TEST_FILE = 'result/morphingdb_stanford_dogs_vector_test.json'
timing_data_image = []
timing_data_vector = []
...
def create_model():
    ...
    model_path = os.path.join(pre_path, 'models/alexnet_stanford_dogs.pt')

    cur.execute("select * from model_info where model_name = 'alexnet_stanford_dogs';")
    res = cur.fetchall()
    if len(res) == 0:
        cur.execute("select create_model('alexnet_stanford_dogs', '{}', '', '');".format(model_path))
...
def stanford_dogs_test():
    for count in IMAGE_COUNT_LIST:
        ...
        start = time.time()
        sql = "select predict_float('alexnet_stanford_dogs', 'cpu', image_vector) from " + IMAGE_VECTOR_TABLE + " limit {};".format(count)
        cur.execute(sql)
        end = time.time()
