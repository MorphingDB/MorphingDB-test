...
def year_predict_test():
    for count in IRIS_COUNT_LIST:
        ...
        start = time.time()
        sql = "select predict_batch_float8('year_predict', 'cpu', data) over (rows between current row and 128 following) from year_predict_test limit {};".format(count)
        cur.execute(sql)
        end = time.time()
