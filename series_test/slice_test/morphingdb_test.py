...
IRIS_TEST_FILE = 'result/morphingdb_test.json'
...
def create_model():
    ...
    model_path = os.path.join(pre_path, 'models/slice.pt')
...
def slice_test():
    for count in IRIS_COUNT_LIST:
        ...
        start = time.time()
        sql = "select predict_batch_float8('slice', 'cpu', data) over (rows between current row and 31 following) from slice_test limit {};".format(count)
        cur.execute(sql)
        end = time.time()
        ...
        try:
            with open(IRIS_TEST_FILE, 'r') as f_vector:
        ...
        with open(IRIS_TEST_FILE, 'w') as f_vector:
